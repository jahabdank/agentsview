...
func TestLegacySourceBaselineColumnMigratesOnce(t *testing.T) {
	path := filepath.Join(t.TempDir(), "archive.db")
	d := testDBAtPath(t, path, "legacy source baseline archive")
	root := t.TempDir()
	eligiblePath := filepath.Join(root, "eligible.jsonl")
	historicalPath := filepath.Join(root, "historical.jsonl")
	insertSessionWithSourcePath(t, d, "eligible", "claude", eligiblePath)
	insertSessionWithSourcePath(t, d, "historical", "claude", historicalPath)
	...
	if legacyColumns == 0 {
		_, err = conn.Exec("ALTER TABLE sessions ADD COLUMN source_baseline_path TEXT")
		require.NoError(t, err)
	}
	_, err = conn.Exec(`
		UPDATE sessions SET source_baseline_path = file_path WHERE id = 'eligible';
		UPDATE sessions SET source_baseline_path = NULL WHERE id = 'historical';
	...
	rows, err := migrated.getReader().Query(`
		SELECT session_id FROM local_session_source_baselines ORDER BY session_id`)
	require.NoError(t, err)
	for rows.Next() {
		var id string
	...
	assert.Equal(t, []string{"eligible"}, baselineIDs,
		"historical NULL ownership must remain ineligible")

	require.NoError(t, migrated.SoftDeleteSession("eligible"))
	restored, err := migrated.RestoreSession("eligible")
	require.NoError(t, err)
	...
	var restoredBaselines int
	require.NoError(t, reopened.getReader().QueryRow(`
		SELECT count(*) FROM local_session_source_baselines
		WHERE session_id = 'eligible'`,
	).Scan(&restoredBaselines))
	assert.Zero(t, restoredBaselines,
		"completed migration must not replay obsolete proof after restore")
}
